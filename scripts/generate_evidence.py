...
def _weight_figure(
    ...
    with figure_style(profile="word_a4", style="fins"):
        ...
        for ax, fund in zip(axes, evidence.COMBINED_FUNDS, strict=True):
            ...
            ax.set_title(WEIGHT_PLOT_LABELS[fund], loc="left", pad=3, fontsize=9)
            ax.set_xlabel("")
        ...
        fig.subplots_adjust(left=0.13, right=0.98, top=0.83, bottom=0.16, hspace=0.90)
        source = (
    ...
    with figure_style(profile="word_a4", style="fins"):
        ...
        for row in performance.itertuples(index=False):
            ...
            ax.scatter(
                ...
                s=72,
                label=(
                    f"{RISK_LEGEND_LABELS[row.fund]} "
                    f"({row.sharpe_ratio:.3g})"
                ),
        ...
        fig.suptitle("Core fund return and risk", x=0.10, ha="left", y=0.985)
        fig.text(0.98, 0.965, "Legend: fund (Sharpe)", ha="right", fontsize=8, color="#4B5563")
        fig.subplots_adjust(left=0.14, right=0.96, top=0.66, bottom=0.25)
        source = (
            "fund_returns.csv; common-period metrics; generate_evidence.py"
        )
        definition = (
            "Common chronological window; sqrt(252) for equity/combined; "
            "sqrt(365) for crypto."
        )
        _add_metadata(
            fig,
...
def _manifest(
    ...
    figure_metadata = {
        ...
        "return_risk_comparison": (
            ...
            "results/data/fund_returns.csv",
            "Common-period geometric annual return versus annualised sample volatility; "
            "equity and combined funds use 252 periods, while crypto funds use 365.",
        ),
        "sector_sentiment_time_series": (
...
def generate_evidence(
    *,
    ...
    comparison_paths = evidence.build_common_period_return_paths(
        sources["fund_returns"]
    )
    comparison_performance = evidence.build_common_period_performance_table(
        sources["fund_returns"]
    )
    ...
    builders = {
        ...
        "return_risk_comparison": (
            _return_risk_figure(comparison_performance),
            len(comparison_performance),
            True,
        ),
        "sector_sentiment_time_series": (
